...
const WindowDetailFooter = ({
  ...
  const savePreparation = (formValue: DB.Window) => {
    ...
    if (formValue.fingerprintConfig) {
      const fingerprint = formValue.fingerprintConfig;
      formValue.fingerprint = JSON.stringify(fingerprint);
      if (fingerprint.ua) formValue.ua = fingerprint.ua;
      delete formValue.fingerprintConfig;
    }
  };
  ...
  const saveWindow = async (formValue: DB.Window) => {
    ...
    if (formValue.id) {
      ...
      if (currentTab === 'windowForm') {
        result = await WindowBridge?.create(formValue, formValue.fingerprint ? JSON.parse(formValue.fingerprint) : fingerprints);
        showMessage(result);
      }
    }
  };
