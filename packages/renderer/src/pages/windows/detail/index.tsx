...
const parseFingerprintConfig = (fingerprint?: string): DB.FingerprintConfig => {
  if (!fingerprint) return {};
  try {
    return JSON.parse(fingerprint) as DB.FingerprintConfig;
  } catch {
    return {};
  }
};

const parseFingerprintReport = (report?: string): DB.FingerprintHealthReport | undefined => {
  if (!report) return undefined;
  try {
    return JSON.parse(report) as DB.FingerprintHealthReport;
  } catch {
    return undefined;
  }
};
...
const WindowDetail = () => {
  ...
  const initFormValue = async () => {
    ...
    if (id) {
      ...
      setFormValue({...window, fingerprintConfig: parseFingerprintConfig(window.fingerprint)});
  ...
  const formValueChangeCallback = (_changed: DB.Window, data: DB.Window) => {
    setFormValue(previous => ({
      ...previous,
      ...data,
      fingerprintConfig: {
        ...previous.fingerprintConfig,
        ...data.fingerprintConfig,
      },
    }));
  };
      ...
      <Card className="window-detail-card">
        {searchParams.get('id') ? (
          <div className="flex w-full mt-4">
            ...
            ></WindowEditForm>
            <FingerprintInfo
              fingerprints={formValue.fingerprintConfig || fingerprints}
               report={parseFingerprintReport(formValue.fingerprint_report || undefined)}
            />
          </div>
        ) : (
          <WindowDetailTabs
