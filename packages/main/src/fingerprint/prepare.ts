...
/**
 * 返回真正用于出口探测的 Axios 代理配置。
 * Mihomo 节点的 proxy 字段是上游 server:port，不是客户端可连接的代理；
 * 浏览器和探测请求必须连接它分配的 127.0.0.1:local_port。
 */
function getLocalProxy(proxy: DB.Proxy): AxiosProxyConfig | undefined {
  // Imported nodes may retain their original protocol in proxy_type
  // (vless/vmess/trojan). node_config + local_port is the authoritative
  // indication that Mihomo owns the connection.
  if (proxy.local_port && (proxy.proxy_type?.toLowerCase() === 'mihomo' || !!proxy.node_config)) {
    return { protocol: 'http', host: '127.0.0.1', port: Number(proxy.local_port) };
  }
  return proxy.proxy && proxy.proxy_type
    ? getRequestProxy(proxy.proxy, proxy.proxy_type)
    : undefined;
}
...
const getRealIP = async (proxy: DB.Proxy) => {
  ...
  if (proxy.proxy_type?.toLowerCase() === 'socks5') {
    ...
    requestProxy = getLocalProxy(proxy);
  }
...
export function getAgent(proxy: DB.Proxy) {
  ...
  if (proxy.proxy) {
    const raw = proxy.proxy.includes('://')
      ? proxy.proxy
      : `${proxy.proxy_type?.toLowerCase() || 'http'}://${proxy.proxy}`;
    ...
    if (!host || !port) return { agent: undefined, agentField };
    switch (proxy.proxy_type?.toLowerCase()) {
      case 'socks5':
        agent = new SocksProxyAgent(
          username ? `socks://${encodeURIComponent(username)}:${encodeURIComponent(password || '')}@${host}:${port}` : `socks://${host}:${port}`,
        );
        agentField = 'httpsAgent';
        break;
      case 'http':
        agent = new HttpProxyAgent(
          username ? `http://${encodeURIComponent(username)}:${encodeURIComponent(password || '')}@${host}:${port}` : `http://${host}:${port}`,
        );
  ...
  for (const pin of PIN_URL) {
    ...
    try {
      const response = await axios.get(pin.url, {
        proxy:
          requestProxy && proxy.proxy_type?.toLocaleLowerCase() !== 'socks5'
            ? requestProxy
            : undefined,
        timeout: 5_000,
        httpAgent: agent,
