...
/**
 * Update only runtime fields.  Keeping this separate from update() is
 * important because getById()/all() return joined display fields and update()
 * intentionally mutates those objects before writing them back.
 */
const updateRuntime = async (id: number, runtime: {
  status: number;
  port?: number | null;
  pid?: number | null;
  opened_at?: string | null;
  local_proxy_port?: number | null;
}) => {
  try {
    await db('window')
      .where({id})
      .update({...runtime, updated_at: db.fn.now()});
    return {success: true, message: 'Window runtime updated successfully.'};
  } catch (error) {
    return {success: false, message: 'Failed to update window runtime.' + error};
  }
};
