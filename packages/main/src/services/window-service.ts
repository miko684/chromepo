...
import {closeFingerprintWindow, openFingerprintWindow, reconcileWindowRuntimeState} from '../fingerprint/index';
...
export const initWindowService = () => {
  ...
  ipcMain.handle('window-close', async (_, id: number, force = false) => {
    // The renderer already requests a forced close.  Keep the fallback here
    // so older renderer/preload bundles also terminate the real process tree.
    return await closeFingerprintWindow(id, force !== false);
  });
